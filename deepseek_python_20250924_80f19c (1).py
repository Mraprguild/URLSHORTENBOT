...
🚀 **Get Started:**
Simply send me any URL and I'll automatically shorten it!
            """
...
📖 **How to use:**
1. Send me any long URL directly (I'll auto-shorten with TinyURL)
2. Or use `/shorten <URL>` command for service options
3. Choose your preferred shortening service from buttons
4. Get your shortened link instantly!

🔗 **Automatic Shortening:**
Just send any URL and I'll instantly shorten it using TinyURL!

🛠 **Supported Services:**

✅ **TinyURL** - Automatic, no API key required (Default)
...
Use `/status` to check your API key configuration.
    ...
    async def status(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            status_text += "\n💡 **Quick Tips:**"
            status_text += "\n• TinyURL works instantly without setup (Auto-mode)"
    ...
    async def handle_message(self, update: Update, context: CallbackContext):
        """Handle messages containing URLs - Auto-shorten with TinyURL + show service buttons"""
        try:
            ...
            # Auto-shorten with TinyURL (always works without API key)
            auto_shortened = self.shorten_url(url, 'tinyurl')
            ...
            if auto_shortened:
                ...
                await update.message.reply_text(
                    f"✅ **Auto-shortened with TinyURL:**\n`{auto_shortened}`\n\n"
                    f"🔗 **Original URL:**\n`{url}`\n\n"
                    f"**Want to use other services? Choose below:**",
                    reply_markup=reply_markup,
                    disable_web_page_preview=True,
                    parse_mode='Markdown'
                )
            else:
                # If auto-shorten fails, show service selection
                await self.process_url(update, url)
